//
// Created by get-coffee on 2/8/25.
//
#include <utility>
#include <vector>
#include <iostream>
...
class BigDecimal {
	public:
	std::vector <bool> integer; //binary, [0] is the smallest
	std::vector <bool> fraction; //binary, [0] is the smallest
	bool sign; // kinda obvious
	unsigned long accuracy; // amount of digits after coma
	...
	static BigDecimal Create(const std::string& input, unsigned long accuracy) {
		...
		// .0123 = 123/10000 0123
		std::reverse(fractionDigits.begin(), fractionDigits.end()); // 3210
...
private:
	static int moduleCompare(BigDecimal l, BigDecimal r) {
...
std::ostream& operator<<(std::ostream& os, const BigDecimal& num) {
 ...
 // to-do: добавить тест кейсы

 // to-do: по возможности / и *
int main() {
	std::string as, bs;
	std::cin >> as;
	std::cin >> bs;
	BigDecimal a = BigDecimal::Create(as, 10);
	BigDecimal b = BigDecimal::Create(bs, 10);
